...
module damping_driver_mod
  ...
  subroutine damping_driver (is, js, lat, Time, delt, pfull, phalf, zfull, zhalf, &
    ...
    if (do_topo_drag) then
      ...
      call topo_drag ( is, js,delt, u, v, t, pfull, phalf, zfull, zhalf,  &
                      lat,z_pbl,utnd,vtnd,ttnd,taubx,tauby,taus,kbot,Time)
  ...
  if( do_mg_drag .and. do_topo_drag) then
    call error_mesg(mod_name, &
      'do_mg_drag and do_topo_drag cannot be both true', FATAL)
  endif
  ...
  !-----------------------------------------------------------------------
  !----- Non-Orographic Gravity Wave Drag (Alexander & Dunkerton) --------
  if (do_cg_drag)  then
  ...
  if (do_topo_drag) then
    call topo_drag_init (lonb, latb, Time,axes)
    sgsmtn(:,:) = -99999.
  endif
